...
mkdir -p /data/ftp/upgrade/dji/log
if [ ! -h /data/dji/log ]; then
	echo "===/data/dji/log is not a link===">>/data/dji/log/upgrade00.log
	if [ -d /data/dji/log ]; then
		echo "===/data/dji/log is a directory, change to link===">>/data/dji/log/upgrade00.log
		busybox mv /data/dji/log /data/ftp/upgrade/dji
		ln -s /data/ftp/upgrade/dji/log /data/dji/log
	else
		echo "===/data/dji/log does not exist===">>/data/dji/log/upgrade00.log
		ln -s /data/ftp/upgrade/dji/log /data/dji/log
	fi
else
	echo "===/data/dji/log is already a link===">>/data/dji/log/upgrade00.log
fi
...
setprop dji.sdrs 1
sleep 3	#wait for /dev/lmi* ready
...
setprop dji.decoding_service 1
...
ps | grep dji_decoding
if [ $? != 0 ];then
	echo "crash_counter: dji_decoding not exist" > /data/dji/log/crash_counter.log
	sync
	exit -1
fi
...
if [ ! -f /amt/new_method_for_fc_log ]; then
	# old customer release
	rm -rf /blackbox/flyctrl/*
	rm -rf /blackbox/vision/*
	mount -o remount,rw /amt
	touch /amt/new_method_for_fc_log
	mount -o remount,ro /amt
fi
rm -rf /blackbox/flyctrl/*.tmp
